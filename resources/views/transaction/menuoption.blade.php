@can('transaction_verify')
    @if (array_key_exists('verify', $button))
        <a href="#" data-url="{{ route($button['verify'], $id) }}"
            ...
            <i class="ph-checks ph-2x text-success"></i>
            Verifikasi
        </a>
    @endif
@endcan
...
@can('transaction_photoshoot')
    @if (array_key_exists('photoshoot', $button))
        <a href="#" data-url="{{ route($button['photoshoot'], $id) }}"
            class="btn flex-column btn-float py-2 mx-2 text-uppercase text-dark fw-semibold"
            onclick="BtnOption('photoshoot',this, event)">
            <i class="ph-arrow-u-up-right ph-2x text-success"></i>
            Sesi Photo
        </a>
    @endif
...
@can('transaction_unphotoshoot')
    @if (array_key_exists('unphotoshoot', $button))
        <a href="#" data-url="{{ route($button['unphotoshoot'], $id) }}"
            class="btn flex-column btn-float py-2 mx-2 text-uppercase text-dark fw-semibold"
            onclick="BtnOption('unphotoshoot',this, event)">
            <i class="ph-arrow-u-up-left ph-2x text-danger"></i>
            Batal Sesi Photo
        </a>
    @endif
